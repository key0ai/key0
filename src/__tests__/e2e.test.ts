...
function makeRouteConfig(): SellerConfig {
	return {
		agentName: "Route Test Agent",
		agentDescription: "Route-first A2A test",
		agentUrl: "https://agent.example.com",
		providerName: "Test Provider",
		providerUrl: "https://provider.example.com",
		walletAddress: WALLET,
		...
		routes: [
			{
				...
				unitAmount: "$0.01",
				description: "Paid weather route",
			},
			{
				routeId: "health",
				method: "GET",
				path: "/health",
				description: "Free health route",
			},
		],
...
describe("E2E: Full Key0 lifecycle (x402 Extension)", () => {
	test("1. AccessRequest → input-required Task with x402 metadata", async () => {
		...
		expect(agentCard.skills[0]!.id).toBe("discover");
		expect(agentCard.skills[1]!.id).toBe("access");
...
describe("E2E: Route-first A2A discovery and access", () => {
	test("discovery returns route catalog alongside plans", async () => {
		const { executor } = createKey0({
		...
		expect(extractTaskState(events)).toBe("completed");
		const catalog = extractChallengeData(events);
	...
	test("paid route access request returns a route-based challenge", async () => {
		const { executor } = createKey0({
			config: makeRouteConfig(),
			adapter: new MockPaymentAdapter(),
			store: new TestChallengeStore(),
		...
		const challenge = extractChallengeData(events);
		expect(challenge["type"]).toBe("X402Challenge");
		expect(challenge["routeId"]).toBe("weather");

		const metadata = extractMetadata(events);
		const paymentRequired = metadata[X402_METADATA_KEYS.REQUIRED] as any;
		expect(paymentRequired.accepts[0].extra.routeId).toBe("weather");
		expect(paymentRequired.accepts[0].extra.planId).toBeUndefined();
	});

	test("free route access returns ResourceResponse without payment", async () => {
		const { executor } = createKey0({
			config: makeRouteConfig(),
			adapter: new MockPaymentAdapter(),
			store: new TestChallengeStore(),
		...
		expect(extractTaskState(events)).toBe("completed");
		const response = extractChallengeData(events);
		expect(response["type"]).toBe("ResourceResponse");
		expect(response["routeId"]).toBe("health");
		expect(response["txHash"]).toBeUndefined();
		expect(response.resource.status).toBe(200);
		expect(response.resource.body.ok).toBe(true);
	});
});
