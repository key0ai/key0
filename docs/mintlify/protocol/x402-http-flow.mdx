...
<Tabs>
  ...
  <Tab title="Case 4: Per-Request Settlement">
    For `mode: "per-request"` plans in standalone mode (seller has `proxyTo` or `fetchResource` configured), include a `resource` field in the body. After payment, Key0 proxies the request to the backend and returns a `ResourceResponse` — **no JWT issued**.
      ...
      "accepts": [
        {
          ...
          "payTo": "0xSellerWallet...",
          "maxTimeoutSeconds": 900,
          "extra": { "name": "USDC", "version": "2", "planId": "weather-query" }
        }
      ],
      "challengeId": "http-a1b2c3d4-...",
      "error": "Payment required"
    ...
    <Note>
      The `resource` field is required for per-request standalone plans. For embedded per-request plans, the `payment-signature` header is sent directly on the route (e.g. `GET /api/weather/london`) — no `/x402/access` involved. See [Paying for Access](/guides/paying-for-access#embedded-per-request).
    </Note>
  </Tab>
</Tabs>
