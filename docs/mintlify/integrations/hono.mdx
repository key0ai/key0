...
`key0App` returns the Hono app **and** a `payPerRequest(planId, opts?)` factory. Use it as Hono middleware on individual routes to gate them behind micro-payments — no JWT issued, payment settles inline.
...
Without a `PAYMENT-SIGNATURE` header the middleware returns `402` with x402 payment requirements. After settlement, the `PaymentInfo` is available via `c.get("key0Payment")`.
