...
CLI binaries are compiled at startup and written to `/app/config/cli-cache` inside the container. Key0 does **not** serve them over HTTP — you are responsible for copying them out and hosting them where your users can download them (e.g. GitHub Releases, S3, a CDN). See the [Agent CLI](/guides/agent-cli) guide for details.
