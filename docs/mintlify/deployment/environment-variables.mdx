...
<Tabs>
  <Tab title="Standalone (Docker)">
    ...
    | Variable | Required | Default | Description |
    |---|---|---|---|
    | `PROXY_TO_BASE_URL` | No | -- | Enable route-based gateway mode. When set, paid top-level `routes` can be called directly: Key0 returns a 402 challenge, proxies the request to this base URL after payment, and returns a `ResourceResponse`. `ISSUE_TOKEN_API` is not called for route purchases. |
