...
```bash Download docker-compose.yml
...
Set `PROXY_TO_BASE_URL` to enable per-request payment gateway mode. After a per-request plan payment is verified, Key0 proxies the request to this base URL and returns the backend response directly to the agent — no `ISSUE_TOKEN_API` call is made for per-request plans.
...
docker run -d \
  ...
  -e PLANS='[{"planId":"weather","unitAmount":"$0.01","mode":"per-request"}]' \
  key0ai/key0:latest
```
