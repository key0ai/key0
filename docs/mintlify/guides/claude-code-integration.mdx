...
<AccordionGroup>
  ...
  <Accordion title="Request fails with 'wrong network'">
    The seller is on a different chain than your wallet. Check `GET /discovery` to see the seller's `network` field (e.g., `eip155:84532` = Base Sepolia, `eip155:8453` = Base mainnet). Ensure your payments-mcp wallet is configured for the same network.
  </Accordion>
