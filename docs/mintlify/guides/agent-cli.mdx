...
<Note>
  In standalone Docker mode, Key0 can host the CLI install flow automatically via `/install.sh` and `/cli/:target`. The manual build/distribute flow below is still useful for embedded deployments or custom distribution.
</Note>
