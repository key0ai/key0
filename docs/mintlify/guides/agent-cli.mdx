...
<Note>
  Key0 does **not** serve CLI binaries over HTTP. In standalone Docker mode, binaries are compiled at startup and written to `/app/config/cli-cache` inside the container — copy them out and host them yourself (GitHub Releases, S3, CDN, etc.). See [Distributing from Docker](#distributing-from-docker) below.
</Note>
...
## Distributing from Docker

In standalone Docker mode, Key0 compiles binaries at startup and writes them to `/app/config/cli-cache` inside the container. Key0 does **not** expose them over HTTP — you must copy them out and host them yourself.
...
**Option B: volume mount** (binaries available on host automatically)
...
Then upload `./cli-dist` to wherever your agents can reach them — GitHub Releases, S3, a CDN, etc.
