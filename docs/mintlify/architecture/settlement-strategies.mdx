...
The `settlePayment()` function is called by all three transports (REST, A2A JSON-RPC middleware, and A2A Executor). It inspects your `SellerConfig` and routes to one of two settlement strategies: **Facilitator Mode** (the default) or **Gas Wallet Mode**.
