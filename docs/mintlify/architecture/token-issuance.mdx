...
<Note>
  **Subscription plans only.** Token issuance applies only to `mode: "subscription"` plans (the default). For `mode: "per-request"` plans, `fetchResourceCredentials` is **not called**:
