...
For `mode: "per-request"` plans, the flow differs slightly: **no JWT is issued**. Instead, Key0 returns the API response directly.

### Standalone (proxy mode)

Add `proxyTo` (or `fetchResource`) to `SellerConfig`. Clients include a `resource` field in the `POST /x402/access` body specifying the route they want to call.
...
Use `key0.payPerRequest(planId)` middleware on each route. No `proxyTo` needed — Key0 settles on-chain and calls `next()`, so your route handler runs as normal.
