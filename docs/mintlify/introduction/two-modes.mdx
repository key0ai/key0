...
<Tabs>
  <Tab title="Standalone (Docker)">
    ...
    - **Pay-per-request (standalone proxy)** is enabled by setting `PROXY_TO_BASE_URL`. For `mode: "per-request"` plans, clients POST to `/x402/access` with a `resource` field; Key0 settles the payment, forwards the request to your backend, and returns the backend response directly as a `ResourceResponse`. No `ISSUE_TOKEN_API` call for per-request plans.
  ...
  <Tab title="Embedded (SDK)">
    ...
    - **Pay-per-request (embedded)** is handled by `key0.payPerRequest(planId)` middleware on individual routes. After on-chain settlement the middleware calls `next()` so your route handler runs normally. No JWT issued; your handler receives `req.key0Payment` with payment metadata.
