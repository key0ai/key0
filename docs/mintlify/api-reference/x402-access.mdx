...
4. **Settlement (per-request standalone)** -- Same as (3) but with a `resource` field in the body for `mode: "per-request"` plans. Key0 proxies to the backend and returns a `ResourceResponse` (backend data, no token).
...
For `mode: "per-request"` plans in standalone mode, also include a `resource` field specifying the route you want to call. Omitting `resource` for a per-request plan in standalone mode returns a `400 RESOURCE_REQUIRED` error.
...
<Tabs>
  <Tab title="Request">
    ...
    **Per-request plan (standalone mode):**
    ...
    | `resourceId` | string | No | Defaults to `"default"`. Ignored for per-request plans. |
    | `resource` | `{ method, path }` | Per-request only | Required for `mode: "per-request"` plans in standalone mode. Specifies the backend route to call after payment. |
  </Tab>
  <Tab title="Response">
...
Resend the same request with a `PAYMENT-SIGNATURE` header containing a base64url-encoded `X402PaymentPayload`. The server settles the payment on-chain and returns either an `AccessGrant` (subscription) or a `ResourceResponse` (per-request standalone).
...
## Case 4: Per-Request Standalone Response

When settlement succeeds for a `mode: "per-request"` plan (with `proxyTo`/`fetchResource` configured), the response is a `ResourceResponse` containing the backend's data — **no JWT is issued**:
...
| 400 | `RESOURCE_REQUIRED` | Per-request plan in standalone mode but `resource` field missing from request body. |
