...
<Warning>
  This example uses the **legacy product/tier API** (`plans`, `tierId`, `onIssueToken`) rather than the current `plans`/`planId`/`fetchResourceCredentials` API. The payment flow is identical -- only the configuration shape differs. See the [Hono integration guide](/integrations/hono) and the [Express Seller example](/examples/express-seller) for the current API.
</Warning>
...
`key0App` returns a Hono sub-app that mounts the A2A agent card at `/.well-known/agent.json` and the JSON-RPC endpoint at `/a2a/jsonrpc`. The `plans` array defines two tiers -- a single photo for \$0.10 and a full album for \$1.00.
