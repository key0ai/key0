...
| Field | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `fetchResourceCredentials` | `(params: IssueTokenParams) => Promise<TokenIssuanceResult>` | Yes | — | Called after on-chain payment is verified for **subscription** plans. Return a token (JWT, API key, etc.) that grants the buyer access. **Not called for per-request plans when `fetchResource` or `proxyTo` is set.** See [IssueTokenParams](#issuetokenparams) and [TokenIssuanceResult](#tokenissuanceresult). |
...
Enable standalone gateway mode for `mode: "per-request"` plans. When set, clients call `POST /x402/access` with a `resource` field; Key0 settles the payment, forwards the request to your backend, and returns a `ResourceResponse`. Both HTTP, A2A, and MCP clients are supported.
...
| Field | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `fetchResource` | `(params: FetchResourceParams) => Promise<FetchResourceResult>` | No | — | Called after settlement for per-request plans. Fetch and return the backend response. See [FetchResourceParams](#fetchresourceparams) and [FetchResourceResult](#fetchresourceresult). |
...
| `mode` | `"subscription" \| "per-request"` | No | `"subscription"` | Billing mode. `"subscription"` issues a JWT; `"per-request"` gates individual API calls without issuing a token. |
| `routes` | `PlanRouteInfo[]` | No | `[]` | Routes guarded by this per-request plan. Used to generate the agent card and populate the discovery response so agents know which paths are available. |
...
| `path` | `string` | Yes | Route path including any Express-style parameters (e.g. `"/api/weather/:city"`). |
...
const config = {
  ...
  // Product catalog — subscription and per-request plans can coexist
  plans: [
