...
let redis: any = null; // Redis client for BullMQ and gas wallet lock (if needed)
...
app.post("/test/expire-request-id", async (req, res) => {
	...
	try {
		...
		if (record.state === "PENDING") {
			...
			return res.json({ success: true, requestId, challengeId: record.challengeId, state: "EXPIRED" });
		}
		...
		return res.json({ success: true, requestId, challengeId: record.challengeId, state: record.state });
	} catch (err) {
		res.status(500).json({ error: String(err) });
	}
});
